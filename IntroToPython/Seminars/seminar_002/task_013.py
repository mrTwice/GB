...
n = int(input('Введите количество дней для анализа среднесуточной температуры: '))
...
for i in lst:
    if i > 0:
        if count > positive: positive = count
        count += 1
    else:
        count = 1
print(f'количество дней со среднесуточной температурой выше нуля: {positive}')